...
model_name = 'ntcd_M2_info_VAD_avd_v3_Lenc_aux_v3_alpha_0.0_beta_10.0_gamma_10.0_y_nonorm_hdim_{:03d}_{:03d}_zdim_{:03d}_end_epoch_{:03d}'.format(h_dim[0], h_dim[1], z_dim, end_epoch)
            ...
            if batch_idx % log_interval == 0:
                print(('Train Epoch: {:2d}   [{:7d}/{:7d} ({:2d}%)]    '\
                    ...
                    + '').format(epoch, batch_idx*len(x), len(train_loader.dataset), int(100.*batch_idx/len(train_loader)),\
                            ELBO.item(), KL.item(), recon_loss.item(), adv_enc_loss.item(), aux_loss.item()), 
