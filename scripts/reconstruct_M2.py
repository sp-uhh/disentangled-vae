...
def main():
    ...
    for i, clean_file_path in tqdm(enumerate(output_clean_file_paths)):
        ...
        # Encode-decode
        reconstruction, _, _ = model(torch.t(S_abs_2), torch.t(y))
        ...
        # Encode-decode (Noisy)
        reconstruction_oracle, _, _ = model(torch.t(X_abs_2), torch.t(y))
        ...
        reconstruction_ones, _, _ = model(torch.t(X_abs_2), torch.t(y_ones))
        ...
        # Encode-decode (Noisy Ones/Zeros)
        reconstruction_zeros, _, _ = model(torch.t(X_abs_2), torch.t(y_zeros))
