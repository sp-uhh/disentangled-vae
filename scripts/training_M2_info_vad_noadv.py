...
model_name = 'ntcd_M2_info_VAD_noadv_Lenc_aux_v3_alpha_10.0_yhatsoft_nofloat_nonorm_hdim_{:03d}_{:03d}_zdim_{:03d}_end_epoch_{:03d}'.format(h_dim[0], h_dim[1], z_dim, end_epoch)
        ...
        for batch_idx, (x, y) in tqdm(enumerate(train_loader)):
            ...
            if batch_idx % log_interval == 0:
                print(('Train Epoch: {:2d}   [{:7d}/{:7d} ({:2d}%)]    '\
                    'ELBO: {:.3f}    KL: {:.3f}    Recon.: {:.3f}    Enc.: {:.3f}    Classif.: {:.3f}    '\
                    + '').format(epoch, batch_idx*len(x), len(train_loader.dataset), int(100.*batch_idx/len(train_loader)),\
                            ELBO.item(), KL.item(), recon_loss.item(), enc_loss.item(), classif_loss.item()), 
