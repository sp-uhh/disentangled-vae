...
from packages.models.mcem import MCEM_M2
from packages.models.models import DeepGenerativeModel, Classifier
...
def process_utt(mcem, model, classifier, mean, std,
                proc_noisy_file_path, clean_file_path, device):
    
    # Check if file exists
    output_path = output_data_dir + proc_noisy_file_path
    output_path = os.path.splitext(output_path)[0] + '_s_est_y_hat_hard.wav'
...
def process_sublist(device, sublist, mcem, model, classifier):
    ...
    for (proc_noisy_file_path, clean_file_path) in sublist:
        process_utt(mcem, model, classifier, mean, std, proc_noisy_file_path, clean_file_path, device)
