...
from packages.models.utils import U_loss, binary_cross_entropy, binary_cross_entropy_v3
...
# model_name = 'ntcd_M2_info_VAD_Uloss_v2_Lenc_aux_v3_alpha_10.0_beta_10.0_gamma_1.0_yhatsoft_nonorm_hdim_{:03d}_{:03d}_zdim_{:03d}_end_epoch_{:03d}'.format(h_dim[0], h_dim[1], z_dim, end_epoch)
# model_name = 'ntcd_M2_info_VAD_Uloss_v2_Lenc_aux_v3_alpha_10.0_beta_30.0_gamma_10.0_yhatsoft_nonorm_hdim_{:03d}_{:03d}_zdim_{:03d}_end_epoch_{:03d}'.format(h_dim[0], h_dim[1], z_dim, end_epoch)
# model_name = 'ntcd_M2_info_VAD_Uloss_v2_Lenc_aux_v3_alpha_10.0_beta_30.0_gamma_20.0_yhatsoft_nonorm_hdim_{:03d}_{:03d}_zdim_{:03d}_end_epoch_{:03d}'.format(h_dim[0], h_dim[1], z_dim, end_epoch)
# model_name = 'ntcd_M2_info_VAD_Uloss_v2_Lenc_aux_v3_alpha_10.0_beta_20.0_gamma_10.0_yhatsoft_nonorm_hdim_{:03d}_{:03d}_zdim_{:03d}_end_epoch_{:03d}'.format(h_dim[0], h_dim[1], z_dim, end_epoch)
# model_name = 'ntcd_M2_info_VAD_Uloss_v2_Lenc_aux_v3_alpha_20.0_beta_20.0_gamma_10.0_yhatsoft_nonorm_hdim_{:03d}_{:03d}_zdim_{:03d}_end_epoch_{:03d}'.format(h_dim[0], h_dim[1], z_dim, end_epoch)
# model_name = 'ntcd_M2_info_VAD_Uloss_v2_Lenc_aux_v3_alpha_10.0_beta_40.0_gamma_10.0_yhatsoft_nonorm_hdim_{:03d}_{:03d}_zdim_{:03d}_end_epoch_{:03d}'.format(h_dim[0], h_dim[1], z_dim, end_epoch)
# model_name = 'ntcd_M2_info_VAD_Uloss_v2_Lenc_aux_v3_alpha_10.0_beta_20.0_gamma_1.0_yhatsoft_nonorm_hdim_{:03d}_{:03d}_zdim_{:03d}_end_epoch_{:03d}'.format(h_dim[0], h_dim[1], z_dim, end_epoch)
model_name = 'ntcd_M2_info_VAD_Uloss_v2_Lenc_aux_v3_alpha_10.0_beta_30.0_gamma_1.0_yhatsoft_nonorm_hdim_{:03d}_{:03d}_zdim_{:03d}_end_epoch_{:03d}'.format(h_dim[0], h_dim[1], z_dim, end_epoch)
...
def main():
    ...
    for epoch in range(start_epoch, end_epoch):
        model.train()
        total_loss, total_U, total_L, total_likelihood, total_kl, total_classif, total_aux, total_aux_ent = (0, 0, 0, 0, 0, 0, 0, 0)
        for batch_idx, (x, y) in tqdm(enumerate(train_loader)):
            ...
            U, L, recon_loss, KL = U_loss(x, r, mu, logvar, y_hat_class_soft, eps)
            ...
            # loss = L - classif_loss
            # loss = U - aux_enc_loss
            loss = U + classif_loss - aux_enc_loss
            ...
            if batch_idx % log_interval == 0:
                print(('Train Epoch: {:2d}   [{:7d}/{:7d} ({:2d}%)]    '\
                    'Total: {:.10f}    U: {:.10f}    L: {:.10f}    Recon.: {:.3f}    KL: {:.3f}    Classif.: {:.3f}    Aux.: {:.3f}    Aux. Ent.: {:.3f}    '\
                    + '').format(epoch, batch_idx*len(x), len(train_loader.dataset), int(100.*batch_idx/len(train_loader)),\
                            loss.item(), U.item(), L.item(), recon_loss.item(), KL.item(), classif_loss.item(), aux_loss.item(), aux_ent_loss.item()), 
        ...
        if epoch % 1 == 0:
            ...
            print("[Train]\t\t Total: {:.3f}, U: {:.3f}, L: {:.3f}, Recon.: {:.3f}, KL: {:.3f}, Classif.: {:.3f}, Aux.: {:.3f}, Aux. Ent.: {:.3f}"\
                "".format(total_loss / t, total_U / t, total_L / t, total_likelihood / t, total_kl / t, total_classif / t, total_aux / t, total_aux_ent / t))

            # Save to log
            print(("Epoch: {}".format(epoch)), file=open(model_dir + '/' + 'output_epoch.log','a'))
            print(("[Train]\t\t Total: {:.3f}, U: {:.3f}, Recon.: {:.3f}, KL: {:.3f}, Classif.: {:.3f}, Aux.: {:.3f}, Aux. Ent.: {:.3f}"\
                "".format(total_loss / t, total_U / t, total_L / t, total_likelihood / t, total_kl / t, total_classif / t, total_aux / t, total_aux_ent / t)),
            ...
            print("[Validation]\t Total: {:.3f}, U: {:.3f}, L: {:.3f}, Recon.: {:.3f}, KL: {:.3f}, Classif.: {:.3f}, Aux.: {:.3f}, Aux. Ent.: {:.3f}"\
                "".format(total_loss / m, total_U / m, total_L / m, total_likelihood / m, total_kl / m, total_classif / m, total_aux / m, total_aux_ent / m))
